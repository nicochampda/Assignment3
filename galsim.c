    ...
    for (i=0; i<nsteps; i++) {
        for (p=0;p<N;p++) {
            ...
            for (j=0;j<N-1;j++) {
                ...
                sum += particules[j].mass / pow(  pow(distancex,2) - pow(distancey, 2) ,2); //utilise la 2e formule pour la stabilite
                }
            }
            F = (-100/N) * particules[p].mass * sum; 
            A = F/particules[p].mass;

            particules[p].pos_x += delta_t*particules[p].vel_x + pow(delta_t,2)*A; // es ce le bon ordre ?
